...
- [x] `start` が `end` より小さい場合、有効な TimeRange として扱える
- [x] `start` が `end` と等しい場合、エラー
- [x] `start` が `end` より大きい場合、エラー
- [x] `start` が負数の場合、エラー
- [x] `name` が空文字列の場合の扱い
  - 空文字列を許可
- [x] `color` が有効な CSS カラー値の場合、正しく保存される
...
- [x] `min` が 0、`max` が 100 の場合、50 は中央にマッピングされる
- [x] `min` が 10、`max` が 20 の場合、15 は中央にマッピングされる
- [x] タイムラインの幅が 1000px の場合、正しいピクセル値に変換される
- [x] 負の値や範囲外の値の扱い
  - 例外を投げる

#### 3.2.2 座標から時間への変換

- [x] タイムラインの左端（0px）が `min` にマッピングされる
- [x] タイムラインの右端（1000px）が `max` にマッピングされる
- [x] 中央の座標がminとmaxの中間の時間値にマッピングされる
- [x] 範囲外の座標の扱い
  - 0未満はminに、width以上はmaxに
